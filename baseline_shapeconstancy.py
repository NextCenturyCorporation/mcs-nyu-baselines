...
def run_shapeconstancy_scene(scene_data, controller, models, base_path="~/logs", num_steps=60, visualize=False, pred_horizon=5):
    report = {}
    ...
    for step in range(num_steps):
        ...
        if motion_start_time is None and background is not None:
            ...
            if x_diff <= MOTION_THRESH:
                first_static = step
        elif step == first_static + 3:
            init_imgs = images[first_static - 10: first_static + 3]
            _, skip = initialize_model(frame_predictor, posterior, prior, encoder, decoder, init_imgs[:5])
        elif step >= 144:
    ...
    detection_interval = range(120, 160)
...
def main(scene_data: dict):
    ...
    NUMSTEPS = 160
    BATCH_SIZE = 1
    model_path = "trained/ShapeConstancyTraining"
    ...
    run_shapeconstancy_scene(scene_data, controller, (frame_predictor, posterior, prior, encoder, decoder), num_steps=NUMSTEPS, visualize=False, pred_horizon=PREDICTION_HORIZON)
