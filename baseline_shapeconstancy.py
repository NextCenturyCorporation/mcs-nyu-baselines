...
def run_shapeconstancy_scene(json_path, controller, models, base_path="~/logs", num_steps=60, visualize=False, pred_horizon=5):
    report = {}
    ...
    for step in range(num_steps):
        ...
        if motion_start_time is None and background is not None:
            ...
            if pred_t_5_steps is not None:
                ...
                if imp_score > 0.5:
                    report[step] = make_step_prediction(choice="implausible", confidence=imp_score,
...
def main(fname: str):
    ...
    run_shapeconstancy_scene(fname, controller, (frame_predictor, posterior, prior, encoder, decoder), num_steps=NUMSTEPS, visualize=False, pred_horizon=PREDICTION_HORIZON)
