...
def model_predict_multi(frame_predictor, prior, encoder, decoder, x_in, skip, t, pred_imgs, pred_horizon):
    with torch.no_grad():
        ...
        # predict t+2 through t+3
        for j in range(2, pred_horizon + 1):
...
def get_polygons_from_img(img, top_n_contours=2, min_area=4*4):
    img = high_pass(img.copy(), min=6, rad=7)
...
def run_scene(scene_data, controller, models, base_path="~/logs", num_steps=60, visualize=False, pred_horizon=3):
    ...
    for step in range(num_steps):
        ...
        # if step > 112:  # manually creates "frozen video" implausibility for testing
        #     images[-1] = images[-2]
        ...
        if motion_start_time is None and background is not None:
            x_diff = cv2.absdiff(source_img, background)
            ...
            # feed the 5 most recent frames, not including this frame, into the model.
            ...
            if step - motion_start_time <= 2:
                ...
                pred_t_1_step = model_predict_multi(frame_predictor, prior, encoder, decoder, images[-2], skip, step - 1, pred_imgs, pred_horizon=pred_horizon)
            ...
            if step in pred_imgs:
                ...
                if visualize:
                    ...
                    # cv2.imshow('prediction t 5 step diff',
                    #            cv2.resize(cv2.absdiff(pred_t_5_steps, background), (384, 384), interpolation=cv2.INTER_NEAREST))
                del pred_imgs[step]
            else:
                pred_t_5_steps = None
                if visualize:
            ...
            # cv2.putText(source_img, 'plausible' if gt_plausible else 'implausible', (10, 10), font, fontScale, 128, thickness)
            if visualize:
            ...
            # cv2.imshow('prediction t 1 step diff',
            #            cv2.resize(pred_1_diff, (384, 384), interpolation=cv2.INTER_NEAREST))

            # cv2.imshow('source diff', cv2.resize(source_diff_marked, (384, 384), interpolation=cv2.INTER_NEAREST))
            # cv2.imshow('pred diff', cv2.resize(pred_diff_marked, (384, 384), interpolation=cv2.INTER_NEAREST))
    ...
    detection_interval = range(80, 180)
    ...
    if motion_start_time is not None:  # if no motion is detected, just say plausible
        for t in detection_interval:
            if predicted_implausible[t]:
                ...
                consecutive_implausible = 0
            if consecutive_implausible == pred_horizon + 2:
                choice = 'implausible'
