...
def model_predict_post(frame_predictor, posterior, encoder, decoder, x_in, x_cur, skip, prior=None):
    """If prior is not None, also conditions the prior on the input"""
    h, _ = encoder(x_in)
...
def run_object_permanence(scene_data, controller, models, base_path="~/logs", num_steps=60, visualize=False, pred_horizon=5):
    report = {}
    frame_predictor, posterior, prior, encoder, decoder = models

    output = controller.start_scene(scene_data)
    MOTION_THRESH = 0.0005
    ...
    for step in range(num_steps):
        ...
        if motion_start_time is None and background is not None:
            x_diff = cv2.absdiff(torch_to_np_img(images[-1]), torch_to_np_img(images[-2]))
            ...
            report[step] = make_step_prediction(choice="plausible", confidence=1)
        elif motion_start_time is not None and first_static is None:
            x_diff = cv2.absdiff(torch_to_np_img(images[-1]), torch_to_np_img(images[-2]))
            x_diff = np.mean(np.float32(x_diff))
            if x_diff <= MOTION_THRESH:
                first_static = step
        elif step == first_static + 3 + 22:
            init_imgs = images[first_static - 7: first_static + 3]
            init_imgs += images[first_static + 3 + 12: first_static + 3 + 22]
            _, skip = initialize_model(frame_predictor, posterior, prior, encoder, decoder, init_imgs)
        elif step >= 200:
            pred_t_1_step = model_predict_multi(frame_predictor, prior, posterior, encoder, decoder, images[-2],
            ...
            if pred_t_5_steps is not None:
                ...
                imp_score, polygon = get_implausibility_score(polys_pred_1, polys_pred_5, thresh=7)
                ...
                if imp_score > 0.5:
                    report[step] = make_step_prediction(choice="implausible", confidence=(1 - imp_score),
                                                    heatmap_img=imp_heatmap.copy())
                    predicted_implausible[step] = True
                else:
                    report[step] = make_step_prediction(choice="plausible", confidence=(1 - imp_score),
                                                    heatmap_img=imp_heatmap.copy())
    ...
    detection_interval = range(180, 230)
    ...
    if motion_start_time is not None:  # if no motion is detected, just say plausible
        for t in detection_interval:
            if predicted_implausible[t]:
                ...
                consecutive_implausible = 0
            if consecutive_implausible == pred_horizon + 4:
                choice = 'implausible'
...
def main(scene_data: dict, unity_app: str = None):
    ...
    run_object_permanence(scene_data, controller, (frame_predictor, posterior, prior, encoder, decoder), num_steps=NUMSTEPS, visualize=False, pred_horizon=PREDICTION_HORIZON)
